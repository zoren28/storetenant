...
export default {
    ...
    methods: {
        fetchUOMDetails(id) {
            axios.get(`/unit_measure/fetch_unit_measure_details/${id}`).then(({data}) => {
                this.UOMForm.fill(data);
            });
        },
        checkUOMForm() {

            this.UOMForm.submit('post', '/unit_measure/update_unit_measure', {
            ...
            .then(({ data, status }) => {
                ...
                swal(
                    ...
                    $("div#modal-edit-unitmeasure").modal('hide');
    ...
    watch: {
        uomDetails:function(id) {
            
            this.fetchUOMDetails(id);
        }
    },
}
</script>
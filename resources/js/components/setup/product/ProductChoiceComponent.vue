        ...
        <div class="page-title-box">
          <div class="page-title-right">
            <ol class="breadcrumb m-0">
              ...
              <li class="breadcrumb-item active">Product Choice</li>
            </ol>
          </div>
          <h4 class="page-title">Product Choice</h4>
        </div>
      </div>
    </div>
    <!-- end page title -->
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            <div class="row">
              <div class="col-md-4">
                <select v-model.number="product_choice" class="form-control">
                  <option value="1">Choice of Drinks</option>
                  <option value="2">Choice of Fries</option>
                  <option value="3">Choice of Sides</option>
                </select>
              </div>
              <div class="col-md-8">
                <button
                  v-if="clickSetup === false"
                  class="btn btn-primary waves-effect waves-light float-right"
                  @click="selectProduct"
                >
                  <i class="fas fa-plus-circle"></i> &nbsp; Select Product
                </button>
                <button
                  v-else
                  class="btn btn-primary waves-effect waves-light float-right"
            ...
            >
              ...
              <tbody>
                <tr v-for="(g, index) in getList" :key="index">
                  ...
                  <td>
                    <img :src="'storage/' + g.image" width="90" height="60" />
                  </td>
                  <td>
                    <button
                      class="btn btn-sm btn-light btn-rounded"
                      :class="'remove-button-' + g.product_id"
                      @click="removeProduct(g.product_id)"
                      data-wenk="Remove Product"
                    >
...
<script>
import SelectProduct from './SelectProductChoiceComponent.vue'
  ...
  methods: {
    async fetchProductList(choice, filter) {
      ...
      const response = await axios.get(
        `/product/fetch_product_choice/${choice}/${filter}`
      )
    ...
    async selectProduct() {
      ...
      const { data } = await axios.get(
        `/product/fetch_product_choice/${this.product_choice}/0`
      )
    ...
    removeProduct(product_id) {
      Swal.fire({
        ...
        if (result.value) {
          ...
          axios
            .put('/product/remove_choice_sides', this.params)
            .then(({ data, status }) => {
              const { message } = data
              Swal.fire({
                type: 'success',

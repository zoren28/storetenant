<template>
    <div class="container">
        <table class="table table-sm table-hover" id="dt-advance-delivery">
            <thead>
                <tr>
                    <th></th>
                    <th>Ticket #</th>
...
export default {
    ...
    methods: {
        fetchAdvanceDeliveryOrder() {
            if ( $.fn.DataTable.isDataTable( '#dt-advance-delivery' ) ) {
                this.table.destroy();
                this.table_data = [];
            }

            axios.get('/advance_order/fetch_advance_delivery/').then((response) => {
                this.table_data = response.data ;
                
                var this_data = this;
                setTimeout(function(){ 

                    this_data.table = $('#dt-advance-delivery').DataTable({
                        destroy : true,
                        "order": [[ 2, "asc" ]],
                        "columnDefs":[  
                            {  
    ...
    mounted() {
        this.fetchAdvanceDeliveryOrder();
    },
    filters: {

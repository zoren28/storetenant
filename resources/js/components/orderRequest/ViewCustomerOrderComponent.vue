                              ...
                              :disabled="
                                (order[0].customer_order_detail !== null &&
                                  order[0].customer_order_detail
                                    .cancelled_status === 1) ||
                                  dateCheck(order) ||
                                  countDiscount(order[0].discount) === 0
                              "
                              @click="
                                requestDiscount(
                                  order[0].ticket_id,
                            ...
                            <div class="form-group">
                              ...
                              <form action="">
                                ...
                                <input
                                  ...
                                  :min="deliverySchedules[1].time_from"
                                  :max="deliverySchedules[1].time_to"
                                />
                              </form>
                            </div>
                          </div>
                        ...
                        <div class="row">
                          ...
                          <div class="col-md-7">
                            <table class="table table-sm" width="100%">
                              ...
                              <tbody>
                                <tr>
                                  <td colspan="2" width="75%">No of Item:</td>
                                ...
                                <tr>
                                  <td colspan="2">
                                    ...
                                    <span
                                      ...
                                      "
                                      class="badge badge-danger"
                                      >Does not reach minimum order</span
                                    >
                                    <span v-else class="badge badge-success"
                                      >Minimum order reached</span
                                    >
                                  </td>
                                  <td align="right">
                                    ₱
                            ...
                            <table class="table table-sm" width="100%">
                              <tbody>
                                <template
                                  v-if="
                                    order[0].customer_discounted_amt.length >
                                      0 && is_discounted === true
                                  "
                                >
                                  <tr
                                ...
                                <tr>
                                  <td width="25%">Rider's Fee</td>
                                  <td width="50%">
                                    <!-- <select
                                      v-model="vehicle"
                                      class="form-control"
                                      @change="vehicleCharges"
                                    >
                                      <option
                                        v-for="v in vehicle_list"
                                        :value="v.id"
                                        :key="v.id"
                                        >{{ v.transpo_name }}</option
                                      >
                                    </select> -->
                                    <input
                                ...
                                <tr>
                                  <td colspan="2">Customer Change</td>
                                  <td align="right">
...
export default {
  name: 'ViewCustomerOrderComponent',
  props: {
    ticketId: {
      type: String,
      required: true
  ...
  methods: {
    ...
    countDiscount(data) {
      ...
      data.forEach(discount => {
        if (discount.discount_storage.global_discount_setup.length > 0) {
          pending++
        }
      })
    ...
    getTenantPrice(data) {
      ...
      this.otherTotalPrice = totalPrice - discounted_amt
    },
    customerDetails(order) {
    ...
    customerTender(data) {
      ...
      for (const bu in data) {
        delivery_charge = data[bu][0].customer_tender.delivery_charge

        if (data[bu][0].vehicle_suggestion !== null) {
          vehicle = data[bu][0].vehicle_suggestion.transpo_id
          vehicle_name = data[bu][0].vehicle_suggestion.vehicle.transpo_name
        }
        tender = parseFloat(data[bu][0].customer_tender.amount)
        customer_address.brgy_id =
          data[bu][0].customer_delivery_info.barangay.brgy_id
        customer_address.town_id =
          data[bu][0].customer_delivery_info.barangay.town_id
        customer_address.prov_id =
          data[bu][0].customer_delivery_info.barangay.town.prov_id
        break
      }
    ...
    itemStatus(data) {
      ...
      for (const item in data) {
        if (data[item].status === 0 && data[item].canceled_status === 1) {
          ...
          this.productDetails(data[item]).stocks == 0 &&
          ...
          parseInt(this.productDetails(data[item]).stocks)
        ) {
          status = true
          break
        } else {
    ...
    async viewProductDetails(order_id, product_id, e, quantity, uom_id) {
      ...
      this.editItemOrderForm = {
        ...
        upgradableItems: {
          uomId: productSubs.upgradable_items.length
            ? productSubs.upgradable_items[0].uom_id
            : null,
          productId: productSubs.upgradable_items.length
            ? productSubs.upgradable_items[0].addon_id
            : null,
          price: productSubs.upgradable_items.length
            ? productSubs.upgradable_items[0].addon_price
            : null
        },
        cSize: {
    ...
    pendingDiscount(discounts, customer_discount) {
      ...
      for (const key in discounts) {
        if (discounts[key].discount_storage.global_discount_setup.length > 0) {
          pending += 1
        }
      }
